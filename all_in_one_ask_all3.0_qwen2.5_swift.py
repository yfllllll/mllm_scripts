...
# 处理每张图片
def process_image(image_path, class_names, output_dir, model, request_config):
    # 检查该图片是否已经处理过
    ...
    # 解析检测结果
    parsed_boxes = parse_response_boxes(response_text)
...
def process_folder(root_dir, folder, saved_folder='pred', model=None, request_config=None, selection_param=1.0, seed=None, dataset_split="all"):
    ...
    for image_path in tqdm(selected_images, desc=f"Processing {dataset_split} dataset", unit="image"):
        process_image(image_path, class_names, pred_folder, model, request_config)

    print(f"All predictions saved in: {pred_folder}")


# 修改后的处理文件夹列表函数，支持灵活设置百分比或影像个数
def process_folders(root_dir, folder_list, saved_folder='pred', model=None,request_config=None, selection_param=1.0, seed=None, dataset_split="all"):
    for i, folder in enumerate(folder_list):
        print(f"Processing folder: {folder}")

        try:
            process_folder(root_dir, folder, saved_folder=saved_folder, model=model,request_config=request_config,
...
if __name__ == "__main__":
    ...
    process_folders(root_dir, folder_list, saved_folder=saved_folder, model=engine, request_config=request_config, selection_param=selection_param, seed=seed, dataset_split=dataset_split)
