...
def plot_bounding_boxes(im, bounding_boxes):
    ...
    print(f"原始边界框输出: {bounding_boxes}")
    bounding_boxes = parse_json(bounding_boxes)
    print(f"解析后的边界框输出: {bounding_boxes}")
...
class QwenVLvLLM:
    ...
    def process_image_with_model(self, image, prompt):
        ...
        try:
            ...
            # 调用 API
            generated_text = call_vllm_chat(messages, temperature=0, max_tokens=9600)

            # 根据输出内容决定如何可视化
            result_image = image.copy()
            if "box_2d" in generated_text:
