...
query_class = "矩形框已在图中绘制出来，结合图像内容，判断矩形框的类别是否属于以下类别集合：猪、马、牛、羊"
...
class YOLOv8QwenVLAnalyzer:
    ...
    def batch_analyze_with_qwen_vl(self, 
        ...
        for batch_idx in range(num_batches):
            ...
            for i, det in enumerate(batch_detections):
                ...
                #将矩形区域绘制到图像上
            maskimg = draw_visual_prompt(image, batch_detections)
            prompt = f"{prompt_template}\n\n" + "\n".join(prompt_parts)
            #回复格式要求
            prompt += "\n只要类别属于类别集合即可回答yes,否则回答no，无需严格判断具体类别. 请严格按照以下格式回复：\n区域1：yes(属于的情况下), brief reason 或 区域1：no（不属于的情况下）, brief reason\n区域2：yes, brief reason 或 区域2：no, brief reason\n..."
            # 准备模型输入
            messages = [
                {
                    "role": "user",
    ...
    def _parse_single_region_response(self, line: str, detection: Dict):
        ...
        Args:
            line: 回答行，如 "区域1： 简短推断理由/yes 或 区域1：简短推断理由/no"
            detection: 检测框字典
        """
...
def main():
    ...
    config = {
        ...
        'qwen_model': '/mnt/disk/lyf/ms-swift/output/test_gkd/v2-20260202-212609/checkpoint-164',  # Qwen-VL模型名称
