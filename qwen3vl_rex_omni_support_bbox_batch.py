...
class AutoDataGroundingAgent:
    ...
    def relabel_boxes_with_qwen(self, image, boxes_data, max_boxes_per_batch=10):
        ...
        try:
            ...
            要求：
            1. label使用中文名词
            2. description用中文短语简要描述该物体的特征、状态等
            3. 每个区域单独一行
    ...
    def _relabel_boxes_batch(self, image, boxes_data, max_boxes_per_batch):
        ...
        for batch_idx in range(total_batches):
            ...
            for i, box in enumerate(batch_boxes):
                ...
                formatted_boxes.append({
                    "bbox_2d": [norm_x1, norm_y1, norm_x2, norm_y2],
                    #"可能包含的目标": label,#str(i+1)  # 使用局部索引（从1开始）
                    "region_id": str(i + 1)
                })
    ...
    def parse_relabel_result(self, relabel_text, original_boxes):
        ...
        for line in lines:
            ...
            region_id_str = parts[0].strip()
            region_info = parts[1].strip()
            
            # 提取区域ID（处理局部索引）
            try:
                region_idx = int(region_id_str) - 1  # 转换为0-based索引
            except ValueError:
                # 如果无法解析ID，跳过这一行
                continue
            
            if region_idx < 0 or region_idx >= len(original_boxes):
                # 索引超出范围，跳过
                continue
            ...
            # 获取对应的原始框数据
            box_data = original_boxes[region_idx].copy()
...
def create_auto_grounding_interface(agent):
    ...
    with gr.Blocks(title="自动数据标注智能体", theme=gr.themes.Soft()) as demo:
        ...
        **工作流程**:
        ...
        with gr.Row():
            with gr.Column():
                ...
                with gr.Accordion("高级配置", open=False):
                    with gr.Row():
                        # 分批处理阈值
                        batch_threshold = gr.Number(
                            label="分批处理阈值",
                            value=10,
                            minimum=1,
                            maximum=50,
                            step=1,
                            info="当矩形框数量超过此值时，将分批处理"
                        )
        ...
        def process_auto_grounding(image, manual_categories_text, use_manual, batch_threshold_value):
            """处理自动数据标注流程"""
            if image is None:
            ...
            try:
                ...
                # 第三步：使用Qwen3-VL重新标定（支持分批处理）
                relabeled_boxes, qwen_relabel_text = agent.relabel_boxes_with_qwen(
                    image, boxes_data, max_boxes_per_batch=batch_threshold_value
                )
        ...
        process_btn.click(
            ...
            inputs=[input_image, manual_categories, use_manual_categories, batch_threshold],
            outputs=[
        ...
        ### 高级配置
        - **分批处理阈值**: 当检测到的矩形框数量超过此值时，系统会自动分批次进行处理，避免输入过长。默认值为10。
        - **手动指定类别**: 可以手动输入想要检测的类别，用逗号分隔。
